...
class val_tt {
  ...
  public:
    ...
    inline void setVal(T x) { _val = x; _isNull = false; }
...
template<class T>
std::ostream&
operator<<(std::ostream& os, const val_tt<T>& x) {
  if(x.isNull()) {
     os << "NULL";
  } else {
    os << x.val();
...
template<class T>
bool 
val_tt<T>::operator==(const T& x) const {
  return (!isNull() && val() == x);
}
...
template<class T>
bool
val_tt<T>::operator==(const val_tt& x) const {
...
template<class T>
val_tt<T>&
val_tt<T>::operator&=(const val_tt<T>& x) {
  if(!x.isNull()) {
    (*this) = x;
  }
  return (*this);
...
template<class T>
val_tt<T>  
val_tt<T>::operator-(const val_tt<T>& x) const {
