...
DateJd::DateJd(const char* s, bool aYearHigh, char sep) : _jd(0) {
  set(s, aYearHigh, sep);
}
...
bool
DateJd::simple_set(const char* aStringPtr, const bool aYearHigh, const char sep) {
...
bool
DateJd::set(const char*& aStringPtr) { // format: YYYYMMDD
  const char* x = aStringPtr;
  char* lEnd;
  uint lDate = 0;
...
void
DateJd::jd2ymd(const uint jd, int& y, int& m, int& d) {
...
// 20170222
void
DateJd::printToString(char* s) const {
  int y = 0;
  int m = 0;
  int d = 0;
  get(y,m,d);
