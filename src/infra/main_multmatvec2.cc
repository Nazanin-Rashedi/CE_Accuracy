...
class Problem {
  ...
  public:
    ...
    inline       uint     bvA4Size() const { return _bvA4Size; }
    inline const uint*    bvA4(const uint i) const { return _bvA4[i].data(); }
    inline       uint     bvA8Size() const { return _bvA8Size; }
    inline const uint*    bvA8(const uint i) const { return _bvA8[i].data(); }
  private:
...
Problem::Problem(const uint aNoBf) 
  ...
  genIdx();
  _bvA4Size = get_n() / 4;
  _bvA4 = new uint_vt[bvA4Size()];
  genBvA4();
  _bvA8Size = get_n() / 8;
  _bvA8 = new uint_vt[bvA8Size()];
  genBvA8();
...
fProdBitA4(double* aVecOut, const Problem& aProblem) {
  ...
  double a,b,c,d;
  double ab, cd;
  const uint lLim = aProblem.bvA4Size();
  ...
  for(uint i = 0; i < lLim; ++i) { 
    ...
    lMat[14] = b + cd;
    lMat[15] = ab + cd;
    const uint* lBv = aProblem.bvA4(i);
    for(uint j = 0; j < m; ++j) {
      aVecOut[j] += lMat[*lBv++];
    }
  }
...
fProdBitA8(double* aVecOut, const Problem& aProblem) {
  ...
  double a,b,c,d,z;
  double ab, cd;
  const uint lLim = aProblem.bvA8Size();
  const double* x = aProblem.vec();
  for(uint i = 0; i < lLim; ++i) {
    a = *x++;
    b = *x++;
    ...
    const uint* lBv = aProblem.bvA8(i);
    for(uint j = 0; j < m; ++j) {
      aVecOut[j] += lMat[*lBv++];
    }
  }
...
Test::exec(const Problem& aProblem) {
  const int aNoRuns = ((aProblem.bf() < 10) ? 4000 : 300);
  Measure lMeasure;
  lMeasure.start();
