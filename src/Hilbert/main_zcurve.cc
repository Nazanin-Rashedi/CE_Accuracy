...
void
print_zrange(uint32_t x, uint32_t y)  {
  std::cout << x << ',' << y;
}
...
class FragmentProcessor {
  public:
    FragmentProcessor(const uint32_t aN) : _n(aN), _vec(aN * aN) {}
  public:
    void processFragment(const ZCurve<uint32_t>::lohi_t& aFragment) {
      for(uint32_t i = aFragment.lo; i <= aFragment.hi; ++i) {
        _vec[i] = true;
      }
    }
    const bool_vt& vec() const { return _vec; }
  public:
    uint32_t _n;
    bool_vt  _vec;
};
...
void
test7(const uint32_t aNoBits, bool aTrace) {
  assert(1 <= aNoBits && 16 >= aNoBits);
  const uint32_t n = (1 << aNoBits); // number of x values, equals number of y values
  ...
  for(uint32_t lYlo = 0; lYlo < n; ++lYlo) {
  for(uint32_t lYhi = lYlo; lYlo < n; ++lYlo) {
    FragmentProcessor lFragmentProcessor(n);
    ZCurve<uint32_t>::recdesc_t lQueryRectangle(lXlo, lYlo, lXhi, lYhi, aNoBits);

    const uint32_t lCount = lZCurve.listFragments(lQueryRectangle, lFragmentProcessor, aTrace);
    ...
    ZCurve<uint32_t>::lohi_t lFrag(3,7);
    if(lVecQ != lFragmentProcessor.vec()) {
      std::cout << "BAD: "
                << "n = "  << n  << ", qr = " 
                // << lQueryRectangle
                // << lFrag
