#include "NR_SamplingAlphaOmegaNewton.hh"
...
unsigned long long 
SamplingAlphaOmegaNewton::combination(unsigned long long n, unsigned long long r) {
    double result = std::exp(std::lgamma(n + 1) - std::lgamma(r + 1) - std::lgamma(n - r + 1));
    return static_cast<unsigned long long>(result);
}

// Calculate the derivative of the PMF with respect to x
double 
SamplingAlphaOmegaNewton::pmfDerivative(unsigned long long N, unsigned long long n, unsigned long long k, double x) {
    double numerator = (combination(k, x - 1) * combination(N - k, n - x + 1)) - (combination(k, x) * combination(N - k, n - x));
    double denominator = combination(N, n);
    return numerator / denominator;
}

// Calculate f'(x) = (1 / P) * P' - 0
double 
SamplingAlphaOmegaNewton::fDerivative(unsigned long long N, unsigned long long n, unsigned long long k, double x) {
    double p = (combination(k, x) * combination(N - k, n - x)) / combination(N, n);
    double pDerivative = pmfDerivative(N, n, k, x);
    return pDerivative / p;
}
...
SamplingAlphaOmegaNewton::alpha(const uint64_t aK) {
  ...
  if(lTrace) {
	...
	return std::ceil(newton_tt(*this, get_initial_value_alpha(aK),1e-4, 0));
}
...
SamplingAlphaOmegaNewton::omega(const uint64_t aK) {
  ...
  init_k(aK);
  return std::max<double>(aK,std::floor(newton_tt(*this, get_initial_value_omega(aK), 1e-4, 0)));
}
